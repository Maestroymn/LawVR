...
namespace AdvancedCustomizableSystem
{
        ...
        private Dictionary<BodyColorPart, Color> bodyColors = new Dictionary<BodyColorPart, Color>()
    {
        ...
        public int beardActiveIndex { get; private set; } = -1;
        public float heightValue { get; private set; } = 0;
        public float headSizeValue { get; private set; } = 0;
        public Vector3 bodyOffset { get; private set; } = Vector3.zero;
        ...
        private Dictionary<string, float> bodyShapeWeight = new Dictionary<string, float>
    {
      ...
      { "Angry", 0 }
        ...
        public CharacterGeneratorSettings CharacterGeneratorSettings;
        ...
        public CharacterCustomizationSetup GetSetup()
        {
            ...
            ccs.Smile = bodyShapeWeight["Smile"];
            ccs.Sadness = bodyShapeWeight["Sadness"];
            ccs.Surprise = bodyShapeWeight["Surprise"];
            ccs.Thoughtful = bodyShapeWeight["Thoughtful"];
            ccs.Angry = bodyShapeWeight["Angry"];
            ...
            SetFaceShapeByArray(new Dictionary<FaceShapeType, float>()
        {
              ...
              { FaceShapeType.Smile,            bodyShapeWeight["Smile"] },
              { FaceShapeType.Sadness,          bodyShapeWeight["Sadness"] },
              { FaceShapeType.Surprise,         bodyShapeWeight["Surprise"] },
              { FaceShapeType.Thoughtful,       bodyShapeWeight["Thoughtful"] },
              { FaceShapeType.Angry,            bodyShapeWeight["Angry"] },
        });
        ...
        public void BakeCharacter(bool includeBlendShapes = true, bool saveMeshes = false)
        {
            ...
            if (bakeGroup == null)
            {
                ...
                bakeGroup.transform.localPosition = defaultGroup.transform.localPosition;
            ...
            {
                if (characterParts[0].skinnedMesh.ElementAtOrDefault(lod_index) == null)
                    continue;
                ...
                if (lod_index == MinLODLevelsCombined)
                {
                    var rootMotion = LOD_rig.AddComponent<RootMotion>();
                    rootMotion.GetRootObject();
                }
            ...
            if (saveMeshes && !Application.isPlaying)
            {
                ...
                cc.characterCustomizationSetup = GetSetup();
                ResetBodyColors();
        ...
        public void RecalculateLOD()
        {
            ...
            else
            {
                ...
                {
                    if (bakeSkinnedMeshRenderers.ElementAtOrDefault(i) != null)
                    {
                        var renderer = new List<SkinnedMeshRenderer>();
        ...
        public void SetLODRange(int minLod, int maxLod)
        {
            ...
            for (var i = minLod; i <= maxLod; i++)
            {
                ...
                obj_animator.applyRootMotion = false;
                if (i == minLod && UseRootMotion)
                {
                    var rootMotion = obj.AddComponent<RootMotion>();
                    rootMotion.GetRootObject();
                }
        ...
        public GameObject CreateNewBakeInstantiate(string name, Transform parent = null)
        {
            ...
            var rootMotion = bakeGroup.GetComponentInChildren<RootMotion>();
            if (rootMotion != null)
                rootMotion.GetRootObject();
        ...
        public void LoadFromFile(string file_name = "")
        {
            ...
            {
                var filePath = string.Format("{0}/characterData_{1}.json", folderPath, (file_name == string.Empty) ? this.name : file_name);
        ...
        public void ResetAll()
        {
            ...
            ResetBodyColors();
            SetBodyShape(BodyShapeType.BreastSize, 0);
            SetBodyShape(BodyShapeType.Fat, 0);
            SetBodyShape(BodyShapeType.Muscles, 0);
            SetBodyShape(BodyShapeType.Slimness, 0);
            SetBodyShape(BodyShapeType.Thin, 0);
            foreach (var bsw in bodyShapeWeight.Keys.ToArray())
            {
                if (Enum.TryParse(bsw, out FaceShapeType faceShape))
                    SetFaceShape(faceShape, 0);
            }
            ...
            SetBeardByIndex(-1);
            SetHairByIndex(0);
            SetElementByIndex(ClothesPartType.Accessory, -1);
            SetElementByIndex(ClothesPartType.Hat, -1);
            SetElementByIndex(ClothesPartType.Pants, -1);
